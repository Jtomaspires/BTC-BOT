...
Define os índices de linha do mesmo CSV (`CNN_PAXG/data/PAXGUSDT-1h-data.csv`) para:
...
3. Correr `CNN_PAXG/backtest/main.ipynb` (idealmente com working directory que permita encontrar `CNN_PAXG/data/` — a primeira célula resolve `CNN_ROOT` subindo diretórios até encontrar `data/PAXGUSDT-1h-data.csv` ou `CNN_PAXG/data/...`).
