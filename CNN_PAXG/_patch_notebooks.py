"""One-off patcher for CNN_PAXG sandbox notebooks. Run from repo: python CNN_PAXG/_patch_notebooks.py"""
import json
import re
...
def _find_cnn_root():
    ...
    for p in [here, *here.parents]:
        if (p / "data" / "PAXGUSDT-1h-data.csv").is_file():
            return p
        cand = p / "CNN_PAXG" / "data" / "PAXGUSDT-1h-data.csv"
        if cand.is_file():
            return (p / "CNN_PAXG").resolve()
    return here
...
def patch_conv1d():
    ...
    i_df, _ = find_cell(lambda t: "read_csv" in t and ("DATA_CSV" in t or "BTCUSDT" in t or "PAXGUSDT" in t))
