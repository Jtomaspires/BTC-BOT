...
class Bot:
    ...
    async def create_exchange(self):
        ...
        # This lets you keep secrets out of source control.
        load_dotenv()
    ...
    async def place_limit_order(self, side: str, is_double_size: bool = False):
        """
        Place a limit order.
        side: 'BUY' or 'SELL'
        Returns the order id string, or None on complete failure.
        """
        dt0 = datetime.now()
        ...
        # Round to exchange precision using loaded market metadata
        order_price = float(self.exchange.price_to_precision(self.symbol, order_price))
        ...
        for _ in range(20):
            try:
                ...
                self.write_to_log(
                    f"{side} limit order placed. "
                    f"price={order_price}  qty={order_size} {self.coin}  "
                    f"duration={datetime.now() - dt0}"
                )
                return order_id
            except Exception as e:
                self.write_to_log(f"Failed to place {side} limit order: {e}")
                await asyncio.sleep(0.4)

        self.write_to_log(f"FAILED to place {side} order after all retries.")
        return None

    async def cancel_order(self, order_id: str):
        """Attempt to cancel order_id with up to 10 retries."""
    ...
    def save(self):
        ...
        with open("./trading_bot/saved_data.pkl", "wb") as f:
            pickle.dump(data, f)

    def load(self):
        try:
            with open("./trading_bot/saved_data.pkl", "rb") as f:
                data = pickle.load(f)
