...
Define os índices de linha do mesmo CSV (`CNN_XRP/data/XRPUSDT-1h-data.csv`) para:
...
3. Correr `CNN_XRP/backtest/main.ipynb` (idealmente com working directory que permita encontrar `CNN_XRP/data/` — a primeira célula resolve `CNN_ROOT` subindo diretórios até encontrar `data/XRPUSDT-1h-data.csv` ou `CNN_XRP/data/...`).
