...
Define os índices de linha do mesmo CSV (`CNN_ETH/data/ETHUSDT-1h-data.csv`) para:
...
3. Correr `CNN_ETH/backtest/main.ipynb` (idealmente com working directory que permita encontrar `CNN_ETH/data/` — a primeira célula resolve `CNN_ROOT` subindo diretórios até encontrar `data/ETHUSDT-1h-data.csv` ou `CNN_ETH/data/...`).
