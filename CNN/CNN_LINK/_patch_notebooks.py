"""One-off patcher for CNN_LINK sandbox notebooks. Run from repo: python CNN_LINK/_patch_notebooks.py"""
import json
import re
...
def _find_cnn_root():
    ...
    for p in [here, *here.parents]:
        if (p / "data" / "LINKUSDT-1h-data.csv").is_file():
            return p
        cand = p / "CNN_LINK" / "data" / "LINKUSDT-1h-data.csv"
        if cand.is_file():
            return (p / "CNN_LINK").resolve()
    return here
...
DATA_CSV = CNN_ROOT / "data" / "LINKUSDT-1h-data.csv"
...
def patch_conv1d():
    ...
    i_df, _ = find_cell(lambda t: "read_csv" in t and ("DATA_CSV" in t or "BTCUSDT" in t or "LINKUSDT" in t))
