...
Define os índices de linha do mesmo CSV (`CNN_LINK/data/LINKUSDT-1h-data.csv`) para:
...
3. Correr `CNN_LINK/backtest/main.ipynb` (idealmente com working directory que permita encontrar `CNN_LINK/data/` — a primeira célula resolve `CNN_ROOT` subindo diretórios até encontrar `data/LINKUSDT-1h-data.csv` ou `CNN_LINK/data/...`).
